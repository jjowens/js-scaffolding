...
function createWorkflow(obj, jsonData) {
    ...
    if (workflow.Actions !== undefined) {
        for(let idx=0; idx < workflow.Actions.length; idx++) {
            let ignoreFlag = workflow.Actions[idx]["ignore"];

            if (ignoreFlag === true) {
                continue;
            }

            let cmds = workflow.Actions[idx]["Commands"];

            for(let idx=0; idx < cmds.length; idx++) {
                let commandIgnoreFlag = cmds[idx]["ignore"];
    
                if (ignoreFlag == undefined || ignoreFlag === true) {
                    continue;
                }
    
                let cmdStr = cmds["command"];
